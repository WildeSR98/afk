...
if false {
    MatchMaking := ContractPageDropdown := ContractJoinDropdown := ModeDropdown := ZA_ApplyConfig := ""
}
...
; Silence IDE/LSP warnings for GUI controls
if false {
    priority1 := priority2 := priority3 := priority4 := priority5 := priority6 := ""
    PlacementPatternDropdown := PriorityUpgrade := StoryDropdown := StoryActDropdown := ModeCancel := ""
    LegendDropdown := LegendActDropdown := RaidDropdown := RaidActDropdown := InfinityCastleDropdown := ""
    UnitExit := NextText := LobbyText := LobbyText2 := mode := NextLevelBox := lastResult := ReturnLobbyBox := ""
    VictoryText := VictoryText2 := DefeatText := DefeatText2 := ""
    Spawn := OpenChat := AreaText := VoteStart := Yen := Ant := Sand := MagicTown := MagicHills := Navy := SnowyTown := ""
    Fiend := Spirit := Academy := SpaceCenter := Mount := Cursed := Nightmare := AirCraft := Hellish := ""
    centerX := centerY := Disconnect := rblxID := MaxText := MaxText2 := UpgradeText := UpgradeText2 := ""
    AutoAbilityBox := AutoOff := XpText := XpText2 := ""
}
...
HandleContractJoin() {
    ...
    if (joinType = "Creating") {
        ...
        FixClick(255, 355)
        Sleep(20000)
        ProcessLog("Waiting 20 seconds for others to join")
        FixClick(400, 460)
    }   else if (joinType = "Joining") {
        ProcessLog("Attempting to join by holding E")
        SendInput("{e down}")
        Sleep(5000)
        SendInput("{e up}")
